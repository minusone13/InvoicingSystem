...
public class StubCommodityList implements Serializable
{
	...
	{// precision 先默认给1，可以达到王雨城所说的算法。若取数字越高，精确度越高，搜索结果数量也就越少
		ArrayList<CommodityPO> result = new ArrayList<CommodityPO>();// CommodityPO
																		// is
																		// changeable
		...
		for (int i = 0; i < flatlist.size(); i++)
			for (int j = 0; j <= s.length() - precision; j++)
				if (!flatlist.get(i).contents(s.substring(j, j + precision)))// the
																				// elements
																				// in
																				// the
																				// list
		...
		for (int i = 0; i < h.size(); i++)
			if (h.get(i) == true)
				result.add(flatlist.get(i).po.clone());// here can also be
														// change
		return result;
	}
