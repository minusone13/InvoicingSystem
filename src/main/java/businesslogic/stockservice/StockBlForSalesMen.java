...
{//这是给进货销售人员层内的接口，由StubStockController实现
	...
	public RM checkIn(String id,String name, String model, int quantity, double price);//当进货单被审批后，请调用
	public RM checkOut(String id,String name, String model, int quantity, double price);//当销售单审批后，请调用
	public RM undoCheckIn(String id,String name, String model, int quantity, double price);//当进货退货单被审批后，请调用
	public RM undoCheckOut(String id,String name, String model, int quantity, double price);//当销售退货单被审批后，请调用
	//上面两个方法会修改库存余量
